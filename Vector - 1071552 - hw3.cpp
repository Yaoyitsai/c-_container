...
vector::vector(const vector& right)
    ...
    myEnd()
{
    const size_type temps = right.myLast - right.myFirst;
    myFirst = new value_type[temps]();

    for (size_type i = 0; i < temps; i++)
        myFirst[i] = right.myFirst[i];

    myEnd = myFirst + temps;
    myLast = myFirst + temps;
}
...
vector& vector::assign( const vector &right )
{
   if( this != &right ) // avoid self-assignment
   {
       ...
       {
           if (capacity() >= rights) //capacity is enough
           {
               for (size_type i = 0; i < rights; i++)
               {
                   myFirst[i] = right.myFirst[i];
           ...
           else if (newc >= rights) //capacity is not enough but new capicity is enough
           {
               delete[] myFirst;
           ...
           else //capacity is not enough and new capicity is still not enough
           {
               delete[] myFirst;
...
void vector::resize( const size_type newSize )
{
    ...
    else // newSize is bigger 
    {
        if (capacity() >= newSize)  // capacity is enough
        {
        ...
        else  //capacity is not enough 
        {
            ...
            value_type newc = capacity() + capacity() / 2;

            newSize > newc ? newc = newSize : newc = newc;

            myFirst = new value_type[newc]{};
            value_type a = 0;

            for (value_type* i = tempPtr; i < myLast; i++, a++)
                myFirst[a] = tempPtr[a];
