   ...
   : myData()
{
    ...
    myData.bx.ptr = myData.myRes > 15 ? new char[myData.myRes + 1] : myData.bx.ptr;
...
void string::resize( const size_type newSize, const char ch )
{
    ...
    if (newSize <= myData.myRes)
    {
        if (newSize < myData.mySize)
            myData.myPtr()[newSize] = '\0';
        else
            for (unsigned int i = myData.mySize; i < newSize; i++)
                myData.myPtr()[i] = ch;
        myData.mySize = newSize;
    }
    // if newsize is bigger than res
    else
    {
        ...
        value_type* tempPtr = new value_type[myData.mySize]; 

        for (size_type i = 0; i < myData.mySize; i++)
            tempPtr[i] = myData.myPtr()[i];

        myData.bx.ptr = new char[newMyRes + 1];

        for (size_type i = 0; i < myData.mySize; i++)
            myData.bx.ptr[i] = tempPtr[i];
